class main:
    """Kelas utama sebagai kontrol aplikasi kasir."""
    def __init__(self):
...
class PembayaranByCard:
    ...
    def hitungTotalHarga(self, total_belanja: float) -> float:
        """Menghitung total harga untuk pembayaran kartu, dengan biaya admin 2.0%."""
        biaya_admin = total_belanja * 0.02
        total_akhir = total_belanja + biaya_admin
        print(f"Biaya Admin (2.0): Rp {biaya_admin:,.2f}")
        return total_akhir
...
class CetakStruk:
    ...
    def __init__(self):
        """Kontrol untuk kelas CetakStruk."""
        pass
...
if __name__ == "__main__":
    # 1. Simulasi Login
    login_attempt = LoginKasir("admin", "admin123")
    if login_attempt.validasiLogin():
        print("=== Login Kasir ===")
        print(f"'{login_attempt.username}' berhasil login.")
        print("-" * 20)

        # 2. Simulasi penambahan item ke keranjang
        print("=== Keranjang Belanja ===")
        keranjang = [
            HitungPembayaran(idMenu="A01", namaMenu="Bakso", harga=10000, jumlah=5),
            HitungPembayaran(idMenu="A03", namaMenu="Es Oyen",harga=7000, jumlah=5)
        ]
        ...
        struk_final = TCetakStruk(noStruk="JKGH2025-001", totalHarga=final_total)
